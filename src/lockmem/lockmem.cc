...
#ifdef DBG
#    define DebugPrint(_fmt_, ...) printf(_fmt_, __VA_ARGS__)
#else
...
bool
ForceLockInWorkingSet(HANDLE Process, PVOID BaseAddress, SIZE_T RegionSize)
{
    for (size_t Tries = 0; Tries < 10; Tries++)
    {
        const NTSTATUS Status = NtLockVirtualMemory(Process, &BaseAddress, &RegionSize, MAP_PROCESS);
        ...
        if (Status == STATUS_WORKING_SET_QUOTA)
        {
            ...
            if (!GetProcessWorkingSetSizeEx(Process, &MinimumWorkingSetSize, &MaximumWorkingSetSize, &Flags))
            {
            ...
            if (!SetProcessWorkingSetSizeEx(Process, MinimumWorkingSetSize, MaximumWorkingSetSize, Flags))
            {
...
bool
Pid2Name(const char *ProcessName, uint32_t &Pid)
{
    PROCESSENTRY32 Pe32;
...
main(int Argc, const char *Argv[])
{
    ...
    const HANDLE Process = OpenProcess(
        PROCESS_SET_QUOTA | PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_READ, false, ProcessId);
    ...
    uint64_t AmountMb = 0;
    for (uint8_t *Address = 0; VirtualQueryEx(Process, Address, &MemoryInfo, sizeof(MemoryInfo));
         Address = (uint8_t *)MemoryInfo.BaseAddress + MemoryInfo.RegionSize)
    {
        ...
        {
            DebugPrint("Skipping %p - %llx because of protect bad bits..\n", BaseAddress, RegionSize);
            continue;
        }
        ...
        {
            DebugPrint("Skipping %p - %llx because of state bad bits..\n", BaseAddress, RegionSize);
            continue;
        }
        ...
        DebugPrint("Read region %p..\r", BaseAddress);
    ...
    //
    // Launch the lockmem-drv if possible.
    //

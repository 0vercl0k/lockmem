...
bool
InstallDriver(const char *ServiceName, const char *ServiceDisplayName, const char *ServiceFilename)
{
    const ScopedServiceHandle_t Scm = OpenSCManager(nullptr, nullptr, SC_MANAGER_CREATE_SERVICE);
    if (Scm == nullptr)
    {
        return false;
    }
...
bool
StartDriver(const char *ServiceName)
{
    const ScopedServiceHandle_t Scm = OpenSCManager(nullptr, nullptr, SC_MANAGER_CREATE_SERVICE);
    ...
    const ScopedServiceHandle_t Service = OpenService(Scm, ServiceName, SERVICE_START);
...
bool
StopDriver(const char *ServiceName)
{
    const ScopedServiceHandle_t Scm = OpenSCManager(nullptr, nullptr, SC_MANAGER_CREATE_SERVICE);
    ...
    const ScopedServiceHandle_t Service = OpenService(Scm, ServiceName, SERVICE_STOP | SERVICE_QUERY_STATUS);
...
bool
RemoveDriver(const char *ServiceName)
{
    const ScopedServiceHandle_t Scm = OpenSCManager(nullptr, nullptr, SC_MANAGER_CREATE_SERVICE);
